...
select 'Null length (-> NULL)' as locateType, '0' as offset,
       [lrs].[STFindPointByLength](linestring,null,0,3,2).STBuffer(0.1) as lengthSegment from data as a
union all
select 'On intersection between seg 1 and 2' as locType,
       'Offset ' + CAST(offset.IntValue as varchar(100)),
      [lrs].[STFindPointByLength](linestring,5.65685424949238,offset.IntValue,3,2).STBuffer(0.2) as lengthSegment
  from data as a
       cross apply
...
select 'Length ' + CAST(cumLength.IntValue as varchar(100)), 
       'Offset ' + CAST(offset.IntValue     as varchar(100)),
       [lrs].[STFindPointByLength](linestring,cumLength.IntValue,offset.IntValue,3,2).STBuffer(0.2) as lengthSegment
  from data as a
       cross apply
...
union all
select 'Length = First Segment Start Point' as locateType,[lrs].[STFindPointByLength](linestring,1,-1.0,3,2).STBuffer(2) as lengthSegment from data as a
union all
select 'Length = First Segment End Point' as locateType,  [lrs].[STFindPointByLength](linestring,5.6,-1.0,3,2).STBuffer(2) as lengthSegment from data as a
union all
select 'Length is middle First Segment' as locateType,    [lrs].[STFindPointByLength](linestring,2.3,-1.0,3,2).STBuffer(2) as lengthSegment from data as a
union all
select 'Length = Second Segment Mid Point' as locateType, [lrs].[STFindPointByLength](linestring,10.0,-1.0,3,2).STBuffer(2) as lengthSegment from data as a
union all
select 'Length = Last Segment Mid Point' as locateType,   [lrs].[STFindPointByLength](linestring,20.0,-1.0,3,2).STBuffer(2) as lengthSegment from data as a
union all
select 'Length = Last Segment End Point' as locateType,   [lrs].[STFindPointByLength](linestring,25.4,-1.0,3,2).STBuffer(2) as lengthSegment from data as a;
GO
...
Print 'Testing [lrs].[STFindPointByRatio] ...';
GO
